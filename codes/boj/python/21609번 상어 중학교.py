...
def get_block_groups(N, table):
  ...
  def visit_block_group(sy, sx, visited):
    ...
    while q:
      ...
      for ny, nx in [(y+1, x), (y-1, x), (y, x+1), (y, x-1)]:
        if 0<=ny<N and 0<=nx<N and not visited[ny][nx] and table[ny][nx] in [0, block_group.num]:
          visited[ny][nx] = True
          q.append((ny,nx))
  ...
  for y in range(N):
    for x in range(N):
      if not visited[y][x] and table[y][x] not in [-1, 0]:
...
def turn_left(N, table):
  ...
  for y in range(N):
    for x in range(N):
      table[N-1-x][y] = table[y][x]

def play(N, table):
  block_groups = get_block_groups(N, table)
  block_groups.sort(key=lambda bg: bg.get_sort_key())
  pop_block_group(table, block_groups[0])
  fall_down(N, table)
  # turn_left(N, table)
  # fall_down(N, table)
